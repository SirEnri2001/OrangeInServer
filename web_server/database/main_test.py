...
def files_init(db: Session = Depends(get_db)):
    file_path=r'E:\PycharmProjects\pythonProject3\audio'
    file_lists = os.listdir(file_path)
    for filename in file_lists:
        absolute_filename = os.path.join(file_path, filename)
        if crud.get_file_by_name(db,absolute_filename) is None:
